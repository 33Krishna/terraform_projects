# Terraform Drift Detection Project Guide
...
This project creates a complete web application infrastructure:

![Architecture Diagram](images/15.1_Diagram.png)
![Architecture Diagram](images/15.2_Diagram.png)
...
git clone https://github.com/33Krishna/15_drift_detection_auto_remediation
cd 15_drift_detection_auto_remediation
...
- **S3 Bucket:** `33Krishna-terraform-state`
...
aws s3 ls s3://33Krishna-terraform-state/
...
2. **Add GitHub Secrets**
   ...
   **Note:** The S3 bucket name (`33Krishna-terraform-state`) is hardcoded in the backend configuration files and doesn't need to be a secret.
...
aws s3 rm s3://33Krishna-terraform-state/dev/terraform.tfstate
aws s3 rm s3://33Krishna-terraform-state/prod/terraform.tfstate

# 2. Delete lock files (if any remain)
aws s3 rm s3://33Krishna-terraform-state/dev/terraform.tfstate.tflock
aws s3 rm s3://33Krishna-terraform-state/prod/terraform.tfstate.tflock

# 3. List remaining files
aws s3 ls s3://33Krishna-terraform-state/ --recursive

# 4. Only if empty and you own the bucket:
aws s3 rb s3://33Krishna-terraform-state --force
```
...
```bash
# List lock files
aws s3 ls s3://33Krishna-terraform-state/ --recursive | grep tflock
...
# Value should be: 33Krishna-terraform-state
```
...
- Dev: `s3://33Krishna-terraform-state/dev/terraform.tfstate`
- Prod: `s3://33Krishna-terraform-state/prod/terraform.tfstate`
