terraform {
  ...
  # Backend configuration example is placed in backend.tf
}

# EC2 instance used for provisioner.
# Each provisioner block is included below but wrapped in block comments (/* ... */).
# For the project, uncomment one provisioner block at a time, then `terraform apply`.
